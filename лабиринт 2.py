...
class MyGame(arcade.Window):
    ...
    def create_keys(self):
        ...
        key.center_x = 50
        key.center_y = SCREEN_HEIGHT - 50

        self.key_list.append(key)

    def create_doors(self):
        door_top = arcade.Sprite(":resources:/images/tiles/doorClosed_top.png", 0.5)
        door_top.center_x = SCREEN_WIDTH - 25
        door_top.center_y = 75
        door_bottom = arcade.Sprite(":resources:/images/tiles/doorClosed_mid.png", 0.5)
        door_bottom.center_x = SCREEN_WIDTH - 25
        door_bottom.center_y = 25
    ...
    def win(self):
        ...
        arcade.draw_text("Вы победили!", 20, 0, arcade.color.BLACK, 18, bold=True)
        arcade.play_sound(arcade.load_sound(":resources:sounds/coin1.wav"))

        self.player.is_win = True
        self.player.speed = 0
    ...
    def on_update(self, delta_time):
        ...
        # Проверяем получение ключа
        keys_hit = arcade.check_for_collision_with_list(self.player, self.key_list)
        for key in keys_hit:
            key.remove_from_sprite_lists()
            self.player.has_key = True

        # Проверяем открытие двери
        doors_hit = arcade.check_for_collision_with_list(self.player, self.door_list)
